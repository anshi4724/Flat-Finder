      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-slate-600 dark:text-slate-400 mt-1 text-sm font-medium">Streamline your property workflow & tenant leads.</p>
          </div>
        ...
        {(!myProperties || myProperties.length === 0) ? (
          <motion.div 
            ...
            className="bg-white dark:bg-white/5 backdrop-blur-3xl rounded-2xl border border-slate-200 dark:border-white/10 p-10 text-center"
          >
            ...
            <h3 className="text-xl font-black text-slate-900 dark:text-white mb-2">No Properties Listed</h3>
            <p className="text-slate-600 dark:text-slate-400 mb-6 max-w-md mx-auto font-medium text-sm">
          ...
          <div className="grid lg:grid-cols-12 gap-8">
            ...
            <div className={`lg:col-span-4 space-y-4 ${viewMode === 'details' ? 'hidden lg:block' : 'block'}`}>
              <div className="text-slate-900 dark:text-white/40 text-xs font-black uppercase tracking-widest mb-4 flex items-center gap-2">
                ...
                <motion.div
                  ...
                  className={`group relative p-3 rounded-2xl border-2 transition-all cursor-pointer ${
                    selectedProperty?._id === property._id 
                      ...
                      : 'bg-white dark:bg-white/5 border-slate-200 dark:border-white/10 hover:bg-slate-100 dark:bg-white/10 hover:border-slate-300 dark:border-white/20'
                  }`}
                >
                  <div className="flex gap-4">
                    ...
                    <div className="overflow-hidden">
                      <h4 className="text-slate-900 dark:text-white font-black text-base truncate group-hover:text-indigo-400 transition-colors">{property.title}</h4>
                      <p className="text-slate-600 dark:text-slate-500 text-[10px] font-bold uppercase tracking-widest">{property.location?.city}</p>
              ...
              <AnimatePresence mode="wait">
                {selectedProperty ? (
                  <motion.div
                    ...
                    animate={{ opacity: 1, x: 0 }}
                    exit={{ opacity: 0, x: -20 }}
                    className="bg-white dark:bg-white/5 backdrop-blur-3xl rounded-[1.5rem] border border-slate-200 dark:border-white/10 p-6 sm:p-8"
                  >
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-6 pb-6 border-b border-white/5">
                      <div className="text-center sm:text-left">
                         <h2 className="text-2xl font-black text-slate-900 dark:text-white mb-1">{selectedProperty.title}</h2>
                         <p className="text-slate-600 dark:text-slate-400 text-sm font-medium">Property leads & feedback.</p>
                      </div>
                      <div className="flex gap-2">
                         <Link 
                           to={`/dashboard/edit-property/${selectedProperty._id}`}
                           className="p-3 rounded-xl bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 text-slate-900 dark:text-white hover:bg-indigo-600 transition-all cursor-pointer"
                         >
                            <HiPencilAlt className="text-lg" />
                         </Link>
                         <button 
                           onClick={() => handleDelete(selectedProperty._id)}
                           className="p-3 rounded-xl bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 text-red-400 hover:bg-red-600 hover:text-slate-900 dark:text-white transition-all cursor-pointer"
                         >
                            <HiTrash className="text-lg" />
                         </button>
                         <button 
                           onClick={() => setViewMode('list')}
                           className="lg:hidden p-3 rounded-xl bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 text-slate-600 dark:text-slate-400 cursor-pointer"
                         >
                            <HiX />
                         </button>
                      </div>
                    ...
                    <div className="grid sm:grid-cols-2 gap-8">
                      ...
                      <div className="space-y-6">
                         <div className="flex items-center gap-2 text-slate-900 dark:text-white font-black text-sm uppercase tracking-widest">
                         ...
                         <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                           {selectedProperty.interestedUsers && selectedProperty.interestedUsers.length > 0 ? (
                             selectedProperty.interestedUsers.map((user) => (
                               <div key={user._id} className="p-4 rounded-2xl bg-white dark:bg-white/5 border border-white/5 hover:border-indigo-500/30 transition-all group">
                                  ...
                                  <div className="space-y-1">
                                     ...
                                     <div className="flex items-center gap-2 text-slate-600 dark:text-slate-400 text-xs text-left">
                             ...
                             <div className="p-10 rounded-3xl border-2 border-dashed border-white/5 text-center">
                                <p className="text-slate-600 dark:text-slate-500 font-medium text-sm">No one has expressed interest yet.</p>
                             </div>
                           )}
                         </div>
                      </div>
                         ...
                         <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                           {selectedProperty.reviews && selectedProperty.reviews.length > 0 ? (
                             selectedProperty.reviews.map((review, i) => (
                               <div key={i} className="p-4 rounded-2xl bg-white dark:bg-white/5 border border-white/5">
                                  <div className="flex justify-between items-center mb-2">
                                     <div className="font-bold text-slate-900 dark:text-white text-sm">{review.user?.name || 'Anonymous'}</div>
                                     <div className="flex gap-0.5">
                                        {[...Array(5)].map((_, star) => (
                                           <HiStar key={star} className={star < review.rating ? "text-amber-400" : "text-slate-900 dark:text-white/10"} />
                                        ))}
                                     </div>
                                  </div>
                             ...
                             <div className="p-10 rounded-3xl border-2 border-dashed border-white/5 text-center">
                                <p className="text-slate-600 dark:text-slate-500 font-medium text-sm">No reviews posted yet.</p>
                             </div>
                           )}
                         </div>
                      </div>
