...
export const PropertyProvider = ({ children }) => {
  ...
  const [isLocationModalOpen, setIsLocationModalOpen] = useState(false); // Changed to false to show content immediately

  // Initial load effect
  useEffect(() => {
    // Always load some featured properties on mount
    fetchFeatured();
  ...
  const fetchFeatured = useCallback(async (city, type) => {
    try {
      ...
      const typesToFetch = type || selectedType || [];
      
      const params = {};
      if (cityToFetch) params.city = cityToFetch;
      if (Array.isArray(typesToFetch) && typesToFetch.length > 0) {
        params.type = typesToFetch.join(',');
      } else if (typesToFetch && typeof typesToFetch === 'string') {
        params.type = typesToFetch;
      }
      
      const { data } = await API.get('/properties/featured', { params });
  ...
  const updateSelectedCity = useCallback((city, type) => {
    if (city !== undefined) {
    ...
    // Call fetchFeatured with the new values
