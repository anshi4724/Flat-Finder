...
const Recommendations = () => {
  ...
  return (
    <section className="py-24 px-4 sm:px-6 lg:px-8 bg-gray-50 dark:bg-[#030712]">
      <div className="max-w-7xl mx-auto">
        {!hasRequested ? (
          <motion.div 
            initial={{ opacity: 0, scale: 0.98 }}
            whileInView={{ opacity: 1, scale: 1 }}
            className="p-12 rounded-[3.5rem] bg-gradient-to-br from-indigo-600/10 via-violet-600/10 to-fuchsia-600/10 border border-slate-200 dark:border-white/10 text-center relative overflow-hidden group"
          >
            ...
            <div className="relative z-10">
              <div className="inline-flex p-4 rounded-3xl bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 text-indigo-400 mb-8 items-center justify-center">
              ...
              <p className="text-slate-600 dark:text-slate-400 text-lg sm:text-xl max-w-2xl mx-auto mb-12 font-medium">
              ...
              <form onSubmit={fetchRecommendations} className="max-w-2xl mx-auto flex flex-col sm:flex-row gap-4">
                <div className="flex-1 relative group/input">
                  <input
                    ...
                    className="w-full px-8 py-6 bg-white dark:bg-white/5 border-2 border-slate-200 dark:border-white/10 rounded-[2rem] text-slate-900 dark:text-white focus:outline-none focus:border-indigo-500/50 transition-all text-lg placeholder:text-slate-600 backdrop-blur-md"
                  />
          ...
          <>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-12">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-4xl font-black text-slate-900 dark:text-white italic tracking-tighter leading-none mb-2">
                    {searchQuery ? (
              ...
              <button
                ...
                className="self-start sm:self-center px-6 py-3 rounded-2xl bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 text-slate-900 dark:text-white text-xs font-black uppercase tracking-widest hover:bg-slate-100 dark:bg-white/10 hover:border-slate-300 dark:border-white/20 transition-all flex items-center gap-2 group"
              >
                Search Again
            ...
            {loading ? (
              <div className="flex gap-8 overflow-x-auto pb-10 custom-scrollbar">
                {[1, 2, 3, 4].map(n => (
                  <div key={n} className="w-80 h-64 bg-white dark:bg-white/5 animate-pulse rounded-[2.5rem] shrink-0 border border-white/5" />
                ))}
              </div>
            ) : recommendations.length === 0 ? (
              <div className="p-20 rounded-[3.5rem] bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 text-center backdrop-blur-3xl">
                <p className="text-slate-600 dark:text-slate-400 text-xl font-medium italic mb-6">No exact matches for <span className="text-slate-900 dark:text-white">"{searchQuery}"</span></p>
                <button
                ...
                {recommendations.map((prop, i) => (
                  ...
                  >
                    <Link to={`/property/${prop._id}`}>
                      <div className="relative aspect-[4/5] rounded-[3rem] overflow-hidden border border-slate-200 dark:border-white/10 group-hover:border-indigo-500/50 transition-all duration-700 shadow-2xl">
                        <img 
                        ...
                        <div className="absolute bottom-10 left-10 right-10 transform group-hover:-translate-y-2 transition-transform duration-500">
                          <h4 className="text-slate-900 dark:text-white font-black text-3xl line-clamp-2 mb-4 tracking-tighter leading-[0.9] group-hover:text-indigo-400 transition-colors uppercase italic">{prop.title}</h4>
                          <div className="flex items-center gap-2 text-slate-700 dark:text-slate-300 font-bold text-sm tracking-tight opacity-70">
                            <HiLocationMarker className="text-indigo-500 text-lg" />
                            <span className="truncate">{prop.location?.area}, {prop.location?.city}</span>
                          </div>
                        </div>
                        
                        <div className="absolute top-8 right-8 px-5 py-3 rounded-[1.5rem] bg-slate-100 dark:bg-white/10 backdrop-blur-xl text-indigo-400 text-lg font-black italic shadow-2xl border border-white/5 group-hover:bg-indigo-600 group-hover:text-white transition-all">
