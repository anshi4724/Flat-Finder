    ...
    >
      <div 
        className={`max-w-7xl mx-auto transition-all duration-500 rounded-2xl ${
          scrolled 
            ? 'bg-white/95 dark:bg-[#030712]/95 backdrop-blur-xl border border-slate-200/50 dark:border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.12)] dark:shadow-[0_8px_32px_rgba(0,0,0,0.5)]' 
            : 'bg-white/90 dark:bg-[#030712]/80 backdrop-blur-xl border border-white/30 dark:border-white/10 shadow-[0_4px_20px_rgba(0,0,0,0.08)] dark:shadow-none'
        }`}
      >
          ...
          <div className="flex items-center gap-4 sm:gap-8">
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all duration-300 group cursor-pointer ${
                scrolled ? 'bg-slate-100 dark:bg-white/10 hover:bg-slate-200 dark:hover:bg-white/20' : 'bg-slate-100/80 dark:bg-white/5 hover:bg-slate-200/80 dark:hover:bg-white/10 backdrop-blur-md'
              }`}
            >
              ...
              <div className="flex flex-col items-start leading-none">
                <span className={`text-[10px] font-black uppercase tracking-widest ${scrolled ? 'text-slate-400 dark:text-slate-500' : 'text-slate-500 dark:text-slate-400'}`}>
                   {selectedType && selectedType.length > 0 ? selectedType.join(', ') : 'Any Type'}
                </span>
                <span className={`text-sm font-black ${scrolled ? 'text-slate-800 dark:text-white' : 'text-slate-700 dark:text-slate-200'}`}>
                  {selectedCity || 'Select City'}
                </span>
              </div>
              <HiChevronDown className={`text-sm ml-1 transition-transform group-hover:translate-y-0.5 ${scrolled ? 'text-slate-400 dark:text-slate-500' : 'text-slate-500 dark:text-slate-400'}`} />
            </button>
          </div>

          {/* Desktop Nav */}
          <div className="hidden lg:flex items-center bg-slate-100/50 dark:bg-white/5 p-1 rounded-2xl border border-slate-200/50 dark:border-white/10 backdrop-blur-sm">
            {navLinks.map((link) => (
              <Link
                key={link.to}
                to={link.to}
                className={`relative flex items-center gap-2 px-5 py-2.5 rounded-xl font-semibold text-sm transition-all duration-300 ${
                  isActive(link.to) 
                    ...
                    : scrolled ? 'text-slate-600 hover:text-indigo-600 dark:text-slate-400 dark:hover:text-indigo-400' : 'text-slate-700 hover:text-indigo-600 dark:text-slate-300 dark:hover:text-indigo-400'
                }`}
              >
                {isActive(link.to) && (
                  <motion.div 
                    layoutId="navbar-active"
                    className="absolute inset-0 bg-white dark:bg-white/10 border border-slate-200/50 dark:border-white/10 shadow-sm rounded-xl"
      ...
      <AnimatePresence>
        {mobileOpen && (
          ...
          >
            <div className="bg-white/95 dark:bg-[#030712]/95 backdrop-blur-2xl border border-slate-200 dark:border-white/10 rounded-3xl shadow-2xl p-4 overflow-hidden">
