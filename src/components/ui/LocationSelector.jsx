    ...
    <AnimatePresence>
      {isLocationModalOpen && (
        <motion.div
          ...
          className="fixed inset-0 z-[999] flex items-center justify-center p-4 bg-gray-50 dark:bg-[#030712]/95 backdrop-blur-3xl"
        >
          <button
            onClick={() => setIsLocationModalOpen(false)}
            className="absolute top-6 right-6 sm:top-10 sm:right-10 p-3 rounded-2xl bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 text-slate-900 dark:text-white hover:bg-slate-100 dark:hover:bg-white/10 transition-all z-[1000] cursor-pointer"
          >
          ...
          <motion.div
            ...
            className="relative w-full max-w-2xl bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-[3.5rem] p-8 sm:p-12 shadow-2xl overflow-hidden"
          >
            {step === 1 ? (
              <>
                <div className="text-center mb-10">
                  <motion.div initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 text-indigo-400 text-xs font-black uppercase tracking-widest mb-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-10">
                  {popularCities.map((city) => (
                    <button
                      ...
                      className="group relative p-6 bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl hover:bg-slate-100 dark:hover:bg-white/10 hover:border-indigo-500 transition-all text-left cursor-pointer"
                    >
                      <div className="text-2xl mb-2 group-hover:scale-125 transition-transform">{city.icon}</div>
                      <div className="font-black text-slate-900 dark:text-white text-xs tracking-widest uppercase">{city.name}</div>
                    </button>
                  ))}
                </div>

                <form onSubmit={handleCustomSubmit} className="relative group">
                  <HiSearch className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-600 dark:text-slate-500 text-2xl group-focus-within:text-indigo-400 transition-colors" />
                  <input
                    type="text"
                    value={customCity}
                    onChange={(e) => setCustomCity(e.target.value)}
                    placeholder="Or enter city manually..."
                    className="w-full pl-16 pr-10 py-5 bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl text-slate-900 dark:text-white placeholder-slate-700 outline-none focus:ring-2 focus:ring-indigo-500/50 font-bold"
                  />
                </form>
              </>
            ) : (
              <>
                <div className="text-center mb-10">
                  ...
                  <p className="text-slate-600 dark:text-slate-400 font-medium text-sm">You can select multiple options relative to your budget.</p>
                </div>
                  ...
                  {propertyTypes.map((type) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className={`font-black text-xs tracking-widest uppercase transition-colors ${isSelected ? 'text-slate-900 dark:text-white' : 'text-slate-600 dark:text-slate-400'}`}>
                          {type.label}
                        </div>
                        {isSelected && (
                ...
                <div className="flex flex-col gap-4">
                  <button
                    ...
                    className="w-full py-5 bg-white dark:bg-indigo-600 text-[#030712] dark:text-white rounded-2xl font-black text-lg transition-all hover:shadow-[0_0_30px_rgba(255,255,255,0.2)] dark:hover:shadow-[0_0_30px_rgba(99,102,241,0.3)] hover:-translate-y-1 active:scale-95 cursor-pointer"
                  >
                    Confirm Selection ({tempTypes.length})
                  </button>
                  <button
                    onClick={() => setStep(1)}
                    className="text-slate-600 dark:text-slate-500 hover:text-slate-900 dark:hover:text-white text-[10px] font-black uppercase tracking-widest transition-colors cursor-pointer"
                  >
                    ← Change Location
                  </button>
                </div>
